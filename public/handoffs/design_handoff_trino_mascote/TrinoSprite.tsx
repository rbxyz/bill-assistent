...
export type SpriteState = {
  ...
  gem?: 0 | 1
  /** bola de futebol (embaixadinhas): centro na grade, ou null */
...
function buildGrid(s: SpriteState): Cell[][] {
  ...
  // ---- bola de futebol (desenhada por cima: está na frente do corpo) ----
  if (s.ball) {
    const bx = s.ball.x
    const by = s.ball.y
    hl(by - 2, bx - 1, bx + 1, 'K')
    ...
    for (const dy of [-1, 0, 1]) {
      put(by + dy, bx - 2, 'K'); put(by + dy, bx + 2, 'K')
      hl(by + dy, bx - 1, bx + 1, 'W')
    }
    put(by, bx, 'K') // gomo central
  }
...
export function TrinoSprite(props: SpriteState & { accent?: Accent }): React.JSX.Element {
  ...
  const colors = useMemo(() => ({ ...BASE_COLORS, ...ACCENTS[accent] }), [accent])
  const rects = useMemo(
    () => toRects(buildGrid(props)),
    [
      props.legs, props.eye, props.pupil, props.hatLift, props.wave, props.aura,
      props.grabbed, props.brow, props.rage, props.gem, props.ball?.x, props.ball?.y
    ]
  )
